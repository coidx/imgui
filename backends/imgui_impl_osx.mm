...
bool ImGui_ImplOSX_HandleEvent(NSEvent* event, NSView* view)
{
    ...
    if (event.type == NSEventTypeKeyDown || event.type == NSEventTypeKeyUp)
    {
        ...
        io_event.NativeKeycode = (int)[event keyCode];
        io_event.Key = ImGui_ImplOSX_KeyCodeToImGuiKey(key_code);
        io_event.Down = event.type == NSEventTypeKeyDown;
    ...
    if (event.type == NSEventTypeFlagsChanged)
    {
        ...
        if (key != ImGuiKey_None)
        {
            ...
            ImGuiIOKeyEvent io_event;
            io_event.Key = key;
            io_event.NativeKeycode = keycode;

            if (mask)
            {
                NSEventModifierFlags modifier_flags = [event modifierFlags];
                io_event.Down = (modifier_flags & mask) != 0;
                io.AddKeyEvent(&io_event);
            }
            else if (imgui_mask)
            {
                io_event.Down = (imgui_flags & imgui_mask) != 0;
                io.AddKeyEvent(&io_event);
            }
        }
