...
// Parameters for io.AddKeyEvent()
struct ImGuiIOKeyEvent
{
    ImGuiKey    Key;                // Dear ImGui key, translated (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
    bool        Down;               // Is the key down?
    int         NativeKeycode;      // Keycode used by native OS or backend, -1 if unknown. Keycode = translated by active keyboard layout. This is currently unused by Dear ImGui.
    int         NativeScancode;     // Scancode used by native OS or backend, -1 if unknown. Scancode = relates to physical keyboard position. This is currently unused by Dear ImGui.
    int         NativeLegacyIndex;  // Override index used for legacy <1.87 IsKeyXXX() functions. Default == NativeKeycode. SDL backend uses NativeScancode.

    ImGuiIOKeyEvent() { Key = ImGuiKey_None; Down = false; NativeKeycode = NativeScancode = NativeLegacyIndex = -1; }
};
